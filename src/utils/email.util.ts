...
export const sendVerificationEmail = async (to: string, otp: string, userName: string) => {
  const htmlContent = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
      <div class="container">
        ...
        <p>This OTP will expire in ${emailConfig.otpExpiryMinutes} minutes. If you didn't request this verification, please disregard this email.</p>
...
export const sendPasswordResetEmail = async (to: string, otp: string, userName: string) => {
  const mailOptions = {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>This OTP will expire in ${emailConfig.otpExpiryMinutes} minutes. If you didn't request this password reset, please ignore this email and ensure your account is secure.</p>
...
export const sendSessionInvitation = async (
  ...
  try {
    await transporter.sendMail(mailOptions);
    logger.info(`Session invitation email sent successfully to ${to}`);
  } catch (error) {
    logger.error('Error sending session invitation email:', error);
